...
draft 必须完整使用这些字段：original, kind(event/relation), target{source,symbol,label,field}, signal{source,symbol,label,field}, controls[], start,end,frequency(daily/weekly/monthly), timing(forward/concurrent), x_transform(return/change/level), y_transform(return/change), lookback,horizon,lag,operator(ge/le),threshold,flat_band,direction(positive/negative/two_sided)。
timing=concurrent 是同期关系：同一天/同一周/同一月，X与Y均计算截至当期的变化，horizon为同期窗口1-20，lookback必须等于horizon，lag必须为0。同日涨跌幅用daily、horizon=lookback=1。用户明确说同日、当天、一起涨跌时必须用concurrent，不能改为次日。timing=forward才观察Y随后几期；不明确同期还是后续时提问。“纳指”需区分纳斯达克综合指数^IXIC、纳斯达克100指数^NDX与ETF QQQ；标普500指数为^GSPC，不能擅自替换为ETF SPY。上涨概率的组间差异目前仅作描述，不能用均值检验的p值宣称概率差异显著。
...
return 是百分数涨跌幅，例如下跌3%用threshold=-3；change 是原单位差，例如利率提高0.1个百分点用0.1；level是原水平。利率、失业率等优先用change。y_transform只允许return/change。forward模式horizon为结果未来窗口1-20，lookback为信号过去窗口1-60，lag为信号额外滞后0-20（后续跨市场关系建议1，但不能保证发布时间）；concurrent按上面的同期定义。day是各序列实际观测日，不是自然日。relation可添加最多2个控制变量，统一使用与信号相同变换/窗口/滞后。event不支持控制变量。所有结论是历史关联。
