...
var reportTemplate = template.Must(template.New("report").Funcs(template.FuncMap{"chart": reportLine}).Parse(`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MarketCheck 检验报告</title><style>body{font-family:system-ui,"Microsoft YaHei";max-width:920px;margin:35px auto;padding:25px;color:#284631;line-height:1.9}h1{font-size:27px}h2{font-size:19px}table{border-collapse:collapse;width:100%;font-size:12px}td,th{padding:8px;text-align:left;border-bottom:1px solid #ddd}.warning{background:#fff5db;padding:12px;margin:12px 0}svg{width:100%;height:auto}svg text{font:11px system-ui;fill:#7c8e6a}pre{white-space:pre-wrap;font:inherit}.summary{padding:18px;background:#eef4e8}</style></head><body><h1>MarketCheck · 市场假设检验</h1><div class="summary">{{.Run.Summary}}</div>{{if .Result.Demo}}<div class="warning">合成演示数据，不代表真实市场。</div>{{end}}<h2>{{.Result.Verdict}}</h2><p>{{.Result.Explanation}}</p><p>{{.Result.ModelDetail}}；有效样本 {{.Result.N}}；{{.Result.Start}} 至 {{.Result.End}}</p>{{range .Result.Series}}<h2>{{.Label}}</h2>{{chart .Points}}{{end}}<h2>AI 解读</h2><pre>{{if .Run.Narrative}}{{.Run.Narrative}}{{else}}本次未请求 AI 解读。{{end}}</pre><h2>数据来源与口径</h2><table><tr><th>标的</th><th>来源</th><th>单位</th><th>口径</th><th>覆盖范围</th></tr>{{range .Result.Sources}}<tr><td>{{.Label}}</td><td>{{.Provider}}</td><td>{{.Units}}</td><td>{{.Adjustment}}</td><td>{{.Start}} — {{.End}}</td></tr>{{end}}</table><h2>局限</h2>{{range .Result.Warnings}}<div class="warning">{{.}}</div>{{end}}<p>检验 ID：{{.Run.ID}}；创建时间：{{.Run.Created}}</p></body></html>`))
...
func (a *App) exportRun(w http.ResponseWriter, r *http.Request, id, kind string) {
	...
	if kind == "report" {
		...
		_ = reportTemplate.Execute(w, map[string]any{"Run": run, "Result": result})
		return
	}
	var columns []string
	rows := [][]string{}
